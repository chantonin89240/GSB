...
class PdoGsb
{
	...
	/**
	 * Retourne les informations d'un visiteur ou comptable
	...
	/**
	 * Retourne les mois sans doublons

	 * @return un tableau associatif de clé un mois -aaaamm- et de valeurs l'année et le mois correspondant
	 */
	public function getLesMoisFiche()
	{
		$req = "SELECT DISTINCT fichefrais.mois AS mois FROM fichefrais WHERE idEtat= 'CL' ORDER BY fichefrais.mois DESC ";
		$res = PdoGsb::$monPdo->query($req);
		$lesMois = array();
		$laLigne = $res->fetch();
		while ($laLigne != null) {
	 ...
	 * @return la liste des visiteur par rapport au statut visiteur dans l'ordre alphabétique
	 */
	public function getLesVisiteurs($ladate)
	{
		//Recuperation de la premiere date 
		//$DerniereDate= $ladate [0];
		//Recuperation de la derniere date 
		//$D1=$ladate[COUNT($ladate)-1];
		$visi = "SELECT visiteur.id AS id, visiteur.nom AS nom, visiteur.prenom AS prenom FROM visiteur INNER JOIN fichefrais ON id =idVisiteur where mois = :DerniereDate GROUP BY visiteur.id, visiteur.nom ,visiteur.prenom ORDER BY nom ASC";
		$resultat = PdoGsb::$monPdo->prepare($visi);
		//$resultat-> bindValue(':D1',$D1, PDO::PARAM_STR);
		$resultat->bindValue(':DerniereDate', $ladate, PDO::PARAM_STR);
		$resultat->execute();
		$laListeVisi = $resultat->fetchAll();
		return $laListeVisi;
	}


	public function calculFF($idVisiteur, $mois)
	{
		$fraisF = "SELECT SUM(quantite * montant) as totFF FROM lignefraisforfait INNER JOIN fraisforfait ON lignefraisforfait.idFraisForfait = fraisforfait.id WHERE idVisiteur=:idVisit AND mois = :mois";
		$tot1 = PdoGsb::$monPdo->prepare($fraisF);
		$tot1->bindValue(':idVisit', $idVisiteur, PDO::PARAM_STR);
		$tot1->bindValue(':mois', $mois, PDO::PARAM_STR);
		$tot1->execute();
		$FF = $tot1->fetch();
